...
class PoliciesFetcher {
    ...
    async _getURLConnection (myHeader = null, sURLString, Timeout, iContentLen, bulkData) {
        return new Promise((resolve) => {
            this._CONNECTION_TIMEOUT = Timeout * 1000;
            this._READ_TIMEOUT = Timeout * 1000;
            const options = {};
            options.headers = {};
            ...
            if (myHeader !== null) {
                ...
                const iTimeout = myHeader.getConnectTimeout();
                options.method = myHeader.getRequestMethod();
                const iReadTimeout = myHeader.getReadTimeout();
                options.headers['Accept-Charset'] = myHeader.getCharset();
                options.headers['Content-Length'] = myHeader.getContentLength();
                this._CONNECTION_TIMEOUT = iTimeout * 1000;
                this._READ_TIMEOUT = iReadTimeout * 1000;
            }
    ...
    async fetchPolicies (onSystemRequest, headers) {
        ...
        if (onSystemRequest.getRequestType() === 'HTTP') {
            ...
            if (response !== null) {
                ...
                mySystemRequest.setRequestType('PROPRIETARY');
