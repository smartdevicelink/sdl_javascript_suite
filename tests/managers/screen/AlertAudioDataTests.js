...
module.exports = function (appClient) {
    describe('AlertAudioDataTests', function () {
        ...
        it('testConstructors', function (done) {
            const alertAudioData = new SDL.manager.screen.AlertAudioData();
            alertAudioData.setPlayTone(true);
            Validator.assertTrue(alertAudioData.isPlayTone());

            const alertAudioData1 = new SDL.manager.screen.AlertAudioData('phoneticString', SDL.rpc.enums.SpeechCapabilities.SC_TEXT);
            Validator.assertEquals('phoneticString', alertAudioData1.getPrompts()[0].getText());

            const alertAudioData2 = new SDL.manager.screen.AlertAudioData('spokenString');
            Validator.assertEquals('spokenString', alertAudioData2.getPrompts()[0].getText());

            const alertAudioData3 = new SDL.manager.screen.AlertAudioData(null, null, testAudio);
